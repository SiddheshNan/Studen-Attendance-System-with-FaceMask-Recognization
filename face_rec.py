...
def sendMail():
    ...
    for key in student_attn:
        txt += f"{index}: {key} present at {student_attn[key]}\n"
        index += 1
    print("Sending Email.. Please wait..")
    mailgun.mail(email, "face mask attendance iot", txt)
        ...
        for box in zip(total_boxes):
            for face in box:
                ...
                if prediction[1] >= prediction_level:
                    ...
                    if names[prediction[0]] not in student_check:
                        student_attn[names[prediction[0]]] = str(datetime.now().strftime("%Y/%m/%d %H:%M:%S"))
                        student_check.append(names[prediction[0]])
                        print(names[prediction[0]], ": is present")
